...
" === First time load of the IDE plugin ==============================<<<
...
function! s:IDE_RecordWindowSize_au()
	if s:IDE_isToggled == 0 && winnr() == bufwinnr(g:IDE_Title)
...
function! s:IDE_ToggleOpen()
	call s:IDE_TraceLog('IDE_ToggleOpen(): window width = '.s:IDE_userWindowWidth)
...
function! s:IDE_UpdateSyntax()
	...
	call s:IDE_LogMsg("IDE syntax for ide-project ".s:IDE_ProjectName." has been updated.")
		...
		while l:last_buffer > 0
			...
			if bufexists(l:last_buffer) && bufloaded(l:last_buffer)
				...
				if has_key(s:IDE_BufferMap,l:bufname)
                    ...
                    " source the syntax highlighting file so the text in the 
                    " buffer will be the correct color
...
function! s:IDE_RetrieveSyntaxScript()
    ...
    let l:syntaxScript=findfile(s:IDE_syntaxScript, s:IDE_ProjectSyn['dir'])
	if (l:syntaxScript=='')
...
function! s:IDE_CreateSyntaxFile(verbose)
	...
	if (l:syntaxOn==1)
		...
		if a:verbose
			...
			call s:IDE_LogMsg("Generating syntax file ".s:IDE_ProjectSyn['syntax_esc']." for ".len(s:IDE_File)." files")
		endif
		...
		if v:shell_error
			...
			call s:IDE_LogMsg("Syntax file has been generated")
			if (s:IDE_isSyntaxWorking == 0)
				call s:IDE_InfoMsg("Restarting advanced syntax highlight")
				call s:IDE_LogMsg("Restarting advanced syntax highlight")
			endif
			let s:IDE_isSyntaxWorking = 1
		endif
...
function! s:IDE_OpenOrJumpTo(fname, editcmd)
	...
	if bufexists(_bufNbr)
		...
		" silent exe 'silent! TlistClose'
		" silent exe 'silent! TlistOpen'
		return 0
	else
...
function! s:IDE_SetupFileAutocommand(buffer)
	...
	if s:IDE_isSyntaxWorking
		call s:IDE_LogMsg( 'Sourcing projects syntax file' )
		silent! exe 'source '.s:IDE_ProjectSyn['syntax_esc']
	endif
...
function! s:FoldTextSetup()
	...
	setlocal nobuflisted
	setlocal nowrap
	call s:IDE_TraceLog('user window width = '.s:IDE_userWindowWidth)
	"exe 'setlocal winwidth='.s:IDE_userWindowWidth
	"exe 'setlocal winminwidth='.s:IDE_userWindowWidth
endfunction
">>>
" === is absolute path? ==================================================<<<
...
function! s:DoWindowSetupAndSplit()
	...
	silent! wincmd p
	if l:current == winnr()
		call s:IDE_TraceLog('About to move IDE via cmd2WindowSide: '.s:IDE_cmd2WindowSide)
	...
	if l:current == winnr()
		...
		wincmd p
		exe s:IDE_cmdLocate
		call s:IDE_TraceLog('About to rezie window to: '.s:IDE_userWindowWidth)
		exe 'silent! vertical resize '.s:IDE_userWindowWidth
		wincmd p
	else
	...
	" VIM has the annoying habit of resizing the windows when doing 
	" wincmd H,L,J,K. We have to create the taglist window, move it left one 
	" window slot, resize the IDe then resize the taglist
	if bufwinnr('__Tag_List__') == -1
		call s:IDE_TraceLog('TagList window not open.... about to open and position it')
		exe 'TlistToggle' 
		exe 'silent! wincmd L'
	endif
	wincmd t
...
function! s:DoWindowSetupAndSplit_au()
	...
	elseif(winnr() != 1)
		exe s:IDE_cmdLocate
		" exe 'silent! vertical resize '.s:IDE_userWindowWidth
		call s:IDE_LogMsg( "Resizing window #".winnr()." as part of DoWindowSetupAndSplit_au" );
	endif
	setlocal cursorline
endfunction
